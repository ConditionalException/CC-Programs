...
-- **                 for Big Reactor                  ** --
-- **                Written by krakaen                ** --
-- **          Tweaked by ConditionalException         ** --
-- **                  Video Tutorial                  ** --
...
-- ******** Version 0.11 Changelog (01/01/2023) ********
-- Added support for multiple monitors
-- Automatically refresh the whole GUI every 5 seconds
-- Handle monitor not being connected
-- Change ON/OFF to toggle button with color dependant on activation state
-- Adjust GUI spacings to clean up out-of-bounds issues
-- Need to rework GUI drawing to use dynamic coords based on monitor size


-- you need to give the index to be able to use the program (defaults to 1)
...
function initReactor()
    if pcall(checkVersionTooOld) then
        ...
        if reactors[1] == nil then
            error("The Reactor is not being detected, make sure the connections(modem) are activated. The problem could also be related to chunk protection on some public servers, ask an admin about it."
                , 0)
        end
    else
...
function draw()
    for mon_ind, mon in pairs(monitors) do
        term.redirect(mon)
...
function updateToggleState(m, state)
    if state then
        button[m]["color"] = colors.green
        button[m]["title"] = "ON"
    else
        button[m]["color"] = colors.red
        button[m]["title"] = "OFF"
    end
end

function powerToggle(m, n)
    local reactor = reactors[1]
...
function modifyRods(limit, number)
    ...
    table.insert(lines,
        { controlsSize['inX'], controlsSize['inY'] + (controlsSize['sectionHeight'] * 1) + 4,
...
function addDrawBoxesSingleReactor()
    ...
    local name = "INFOS"
    table.insert(lines,
        { infosSize['startX'] + margin, infosSize['startY'], infosSize['startX'] + (margin * 2) + #name + 1,
            infosSize['startY'], colors.black })
    ...
    for i = 0, 2, 1 do
        table.insert(texts,
            { infosSize['inX'] + margin, infosSize['inY'] + (infosSize['sectionHeight'] * i) + i, names[i + 1],
                colors.white,
                colors.black })
        table.insert(filleds,
            { infosSize['inX'], infosSize['inY'] + 2 + (infosSize['sectionHeight'] * i) + i,
                infosSize['inX'] + infosSize['width'
                    ], infosSize['inY'] + (infosSize['sectionHeight'] * (i + 1)) - 2 + i, colors.lightGray })
    end

    -- Controls
    table.insert(boxes,
        { controlsSize['startX'], controlsSize['startY'], controlsSize['endX'], controlsSize['endY'], colors.gray })
    name = "CONTROLS"
    table.insert(lines,
        { controlsSize['startX'] + margin, controlsSize['startY'], controlsSize['startX'] + (margin * 2) + #name + 1,
            controlsSize['startY'], colors.black })
    table.insert(texts,
        { controlsSize['startX'] + (margin * 2), controlsSize['startY'], name, colors.white, colors.black })
    ...
    local control_margins = 7.2
    table.insert(texts,
        { controlsSize['inX'] + control_margins + 5, controlsSize['inY'] + (controlsSize['sectionHeight'] * 1) + 1,
            'AUTO-CONTROL', colors.white,
            colors.black })

    table.insert(texts,
        { controlsSize['inX'] + control_margins + 0.2, controlsSize['inY'] + (controlsSize['sectionHeight'] * 1) + 3,
            'MIN',
            colors.lightBlue,
            colors.black })
    table.insert(texts,
    ...
    table.insert(texts,
        { controlsSize['inX'] + (control_margins * 2) + 3, controlsSize['inY'] + (controlsSize['sectionHeight'] * 1) + 4,
            ...
            colors.black })
    table.insert(texts,
        { controlsSize['inX'] + control_margins * 3.6, controlsSize['inY'] + (controlsSize['sectionHeight'] * 1) + 3,
            'MAX',
            colors.purple,
            colors.black })
    table.insert(texts,
    ...
    -- Numbers
    table.insert(boxes,
        { numbersSize['startX'], numbersSize['startY'], numbersSize['endX'], numbersSize['endY'], colors.gray })
    name = "NUMBERS"
    table.insert(lines,
        { numbersSize['startX'] + margin, numbersSize['startY'], numbersSize['startX'] + (margin * 2) + #name + 1,
            numbersSize['startY'], colors.black })
...
function refreshSingleReactor()
    ...
    table.insert(lines,
        { numbersSize['inX'], numbersSize['inY'], numbersSize['inX'] + numbersSize['width'], numbersSize['inY'],
            colors.black })
    table.insert(texts,
        { numbersSize['inX'], numbersSize['inY'] + 1, infotoAdd .. rfPerTick .. " RF", colors.white, colors.black })
    table.insert(filleds,
        { infosSize['inX'], infosSize['inY'] + 1 + (infosSize['sectionHeight'] * i) + i,
            infosSize['inX'] + infosSize['width'],
            infosSize['inY'] + (infosSize['sectionHeight'] * (i + 1)) - 2 + i, colors.lightGray })

    width = math.floor((infosSize['width'] / rfPerTickMax) * rfPerTick)
    table.insert(filleds,
        { infosSize['inX'], infosSize['inY'] + 1 + (infosSize['sectionHeight'] * i) + i, infosSize['inX'] + width,
            infosSize['inY'] + (infosSize['sectionHeight'] * (i + 1)) - 2 + i, colors.green })
    ...
    table.insert(filleds,
        { infosSize['inX'], infosSize['inY'] + 1 + (infosSize['sectionHeight'] * i) + i, infosSize['inX'] + width,
            infosSize['inY'] + (infosSize['sectionHeight'] * (i + 1)) - 2 + i, colors.green })
    table.insert(lines,
        { numbersSize['inX'], numbersSize['inY'] + 2, numbersSize['inX'] + numbersSize['width'], numbersSize['inY'] + 2,
            colors.black })
    table.insert(texts,
        { numbersSize['inX'], numbersSize['inY'] + 3, infotoAdd .. rfTotal .. " RF", colors.white,
            colors.black })
    ...
    table.insert(filleds,
        { infosSize['inX'], infosSize['inY'] + 1 + (infosSize['sectionHeight'] * i) + i, infosSize['inX'] + width,
            infosSize['inY'] + (infosSize['sectionHeight'] * (i + 1)) - 2 + i, colors.green })
    table.insert(lines,
        { numbersSize['inX'], numbersSize['inY'] + 4, numbersSize['inX'] + numbersSize['width'], numbersSize['inY'] + 4,
            colors.black })
    table.insert(texts,
        { numbersSize['inX'], numbersSize['inY'] + 5, infotoAdd .. rodLevel .. "%", colors.white, colors.black })

    i = 3
    infotoAdd = 'FUEL USAGE : '

    table.insert(lines,
        { numbersSize['inX'], numbersSize['inY'] + 6, numbersSize['inX'] + numbersSize['width'], numbersSize['inY'] + 6,
            colors.black })
    table.insert(texts,
        { numbersSize['inX'], numbersSize['inY'] + 7, infotoAdd .. format_num(tonumber(fuelPerTick), 3) .. "mb/t",
            colors.white,
            colors.black })
    ...
    updateToggleState("TOGGLE", reactor.getActive())
    draw()
...
-- Determine margins from monitor size
function getMargins()
    local w, h = monitors[1].getSize()
    margin = math.floor((w / 100) * 2)

    infosSize['startX'] = margin + 1
    infosSize['startY'] = margin + 1
    infosSize['endX'] = (((w - (margin * 2)) / 4) * 2) - margin
    ...
    setButton("TOGGLE", "ON", powerToggle,
        controlsSize['inX'],
        controlsSize['inY'] + 1,
        controlsSize['inX'] + controlsSize['width'] + 1,
        controlsSize['inY'] + 3,
        "TOGGLE", 0, colors.green)

    setButton("low-10", "-10", modifyRods,
        controlsSize['inX'],
        controlsSize['inY'] + (controlsSize['sectionHeight'] * 2) + 2,
        controlsSize['inX'] + math.floor(controlsSize['width'] / 2) - 1,
        controlsSize['inY'] + (controlsSize['sectionHeight'] * 2) + 4,
        "min", -10, colors.lightBlue)

    setButton("high-10", "-10", modifyRods,
        controlsSize['inX'] + math.floor(controlsSize['width'] / 2) + 2,
        controlsSize['inY'] + (controlsSize['sectionHeight'] * 2) + 2,
        controlsSize['inX'] + controlsSize['width'] + 1,
        controlsSize['inY'] + (controlsSize['sectionHeight'] * 2) + 4,
        "max", -10, colors.purple)

    setButton("low+10", "+10", modifyRods,
        controlsSize['inX'],
        controlsSize['inY'] + (controlsSize['sectionHeight'] * 3) + 2,
        controlsSize['inX'] + math.floor(controlsSize['width'] / 2) - 1,
        controlsSize['inY'] + (controlsSize['sectionHeight'] * 3) + 4,
        "min", 10, colors.lightBlue)
...
-- Clear monitors and restore configuration
